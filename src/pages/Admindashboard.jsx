...
// /* ── ICONS ── */
// const IconUsers   = ()=><svg viewBox="0 0 24 24" fill="black" width="22" height="22"><path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z"/></svg>;
// const IconHeart   = ()=><svg viewBox="0 0 24 24" fill="black" width="22" height="22"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>;
// const IconBookmark= ()=><svg viewBox="0 0 24 24" fill="black" width="22" height="22"><path d="M17 3H7c-1.1 0-1.99.9-1.99 2L5 21l7-3 7 3V5c0-1.1-.9-2-2-2z"/></svg>;
// const IconShots   = ()=><svg viewBox="0 0 24 24" fill="black" width="22" height="22"><path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"/></svg>;
...
// const IconTrash   = ()=><svg viewBox="0 0 24 24" fill="black" width="15" height="15"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>;
// const IconPromote = ()=><svg viewBox="0 0 24 24" fill="black" width="15" height="15"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>;
// const IconRefresh = ()=><svg viewBox="0 0 24 24" fill="black" width="16" height="16"><path d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/></svg>;
// const IconSearch  = ()=><svg viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="2" width="16" height="16"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4.35-4.35"/></svg>;
// const IconBack    = ()=><svg viewBox="0 0 24 24" fill="black" width="18" height="18"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg>;
...
//           {user.role !== "ADMIN" && (
//             <button className="ad-action-btn ad-action-promote" onClick={() => onPromote(user.id)} title="Promote to Admin">
//               <IconPromote/> Promote
//             </button>
//           )}
//           <button className="ad-action-btn ad-action-delete" onClick={() => onDelete(user.id)} title="Delete user">
//             <IconTrash/>
//           </button>
//         </div>
//       </td>
...
//         fetch("http://localhost:8080/api/admin/likes",  { headers: { Authorization: `Bearer ${token()}` } }),
//         fetch("http://localhost:8080/api/admin/saves",  { headers: { Authorization: `Bearer ${token()}` } }),
//       ]);
...
//       setStats({ totalUsers:6, totalShots:16, totalLikes:24, totalSaves:12 });
//       setUsers([
//         { id:1, name:"Admin",          email:"admin@dribbble.com",   role:"ADMIN", likedCount:5,  savedCount:3,  createdAt: new Date().toISOString() },
//         { id:2, name:"Jeroen van Eerden",email:"jeroen@example.com", role:"USER",  likedCount:8,  savedCount:2,  createdAt: new Date().toISOString() },
//         { id:3, name:"Kunal Studio", email:"Kunal@example.com",role:"USER",likedCount:11, savedCount:7,  createdAt: new Date().toISOString() },
//         { id:4, name:"Tanbir Design",      email:"Tanbir@example.com",   role:"USER",  likedCount:0,  savedCount:0,  createdAt: new Date().toISOString() },
//       ]);
//     }
//     setLoading(false);
//   };
...
//     try {
//       const res = await fetch(`http://localhost:8080/api/admin/users/${userId}/promote`, {
//         method: "PUT",
...
//             <h3>Delete User?</h3>
//             <p>This will permanently delete the user and all their likes and saves. This cannot be undone.</p>
...
//             </h1>
//             <p className="ad-header-sub">Manage users, monitor activity, and control the platform.</p>
//           </div>
//           <div className="ad-header-actions">
//             <button className="ad-btn-refresh" onClick={fetchAll} disabled={loading}>
//               <IconRefresh /> {loading ? "Loading…" : "Refresh"}
//             </button>
//             <Link to="/dashboard" className="ad-btn-back">
//               <IconBack /> My Profile
//             </Link>
//           </div>
//         </div>
...
//             <StatCard label="Total Shots"  value={stats.totalShots}  color="#0ea5e9" icon={<IconShots/>}    sub="Design shots uploaded" />
...
//                       <span className="ad-count-badge ad-count-pink" title="Likes">{u.likedCount ?? 0} ♥</span>
//                       <span className="ad-count-badge ad-count-dark" title="Saves">{u.savedCount ?? 0} 🔖</span>
//                     </div>
//                   );
//                 })}
//               </div>
...
//         {activeTab === "likes" && (
//           <div className="ad-section">
//             <h2 className="ad-section-title">All Like Records</h2>
//             {loading ? <div className="ad-loading">Loading likes…</div> : likes.length === 0 ? (
...
//                       <th className="ad-th">User</th>
//                       <th className="ad-th">Shot</th>
//                       <th className="ad-th">Liked At</th>
//                     </tr>
//                   </thead>
//                   <tbody>
//                     {likes.map((l, i) => (
//                       <tr key={l.likeId} className="ad-table-row">
//                         <td className="ad-td ad-td-muted">{i + 1}</td>
//                         <td className="ad-td">
//                           <div style={{display:"flex",flexDirection:"column",gap:1}}>
//                             <span className="ad-user-name">{l.userName}</span>
//                             <span className="ad-user-email">{l.userEmail}</span>
//                           </div>
//                         </td>
//                         <td className="ad-td"><span className="ad-shot-title">{l.shotTitle}</span></td>
//                         <td className="ad-td ad-td-muted">{l.likedAt ? new Date(l.likedAt).toLocaleString() : "—"}</td>
//                       </tr>
//                     ))}
//                   </tbody>
//                 </table>
...
//         {activeTab === "saves" && (
//           <div className="ad-section">
//             <h2 className="ad-section-title">All Save Records</h2>
//             {loading ? <div className="ad-loading">Loading saves…</div> : saves.length === 0 ? (
...
//                       <th className="ad-th">User</th>
//                       <th className="ad-th">Shot</th>
//                       <th className="ad-th">Saved At</th>
//                     </tr>
//                   </thead>
//                   <tbody>
//                     {saves.map((sv, i) => (
//                       <tr key={sv.saveId} className="ad-table-row">
//                         <td className="ad-td ad-td-muted">{i + 1}</td>
//                         <td className="ad-td">
//                           <div style={{display:"flex",flexDirection:"column",gap:1}}>
//                             <span className="ad-user-name">{sv.userName}</span>
//                             <span className="ad-user-email">{sv.userEmail}</span>
//                           </div>
//                         </td>
//                         <td className="ad-td"><span className="ad-shot-title">{sv.shotTitle}</span></td>
//                         <td className="ad-td ad-td-muted">{sv.savedAt ? new Date(sv.savedAt).toLocaleString() : "—"}</td>
//                       </tr>
//                     ))}
//                   </tbody>
//                 </table>
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const [statsRes, usersRes, likesRes, savesRes] = await Promise.all([
        ...
        fetch("http://localhost:8080/api/admin/likes", { headers }),
        fetch("http://localhost:8080/api/admin/saves", { headers }),
      ]);
      if (statsRes.ok) setStats(await statsRes.json());
      if (usersRes.ok) setUsers(await usersRes.json());
      if (likesRes.ok) setLikes(await likesRes.json());
